...
WebMessenger.enums.EventType = {ON_RECEIVE_HISTORY:"ON_RECEIVE_HISTORY", ON_RECEIVE_MESSAGE:"ON_RECEIVE_MESSAGE"};
goog.events = {};
...
WebMessenger.common = {};
WebMessenger.common.HttpRequest = function(a, b, c, d, e) {
  goog.isFunction(b) && (this.onSuccessCallback_ = b);
  goog.isFunction(c) && (this.onErrorCallback_ = c);
  goog.isFunction(d) && (this.onAbortCallback_ = d);
  goog.isFunction(e) && (this.onReadyStateChange_ = e);
  this.sender_ = a;
...
WebMessenger.common.HttpRequest.prototype.onSuccessCallback_ = function(a, b) {
};
WebMessenger.common.HttpRequest.prototype.onErrorCallback_ = function(a, b) {
};
WebMessenger.common.HttpRequest.prototype.onAbortCallback_ = function(a, b) {
};
WebMessenger.common.HttpRequest.prototype.onReadyStateChange_ = function(a, b, c) {
};
WebMessenger.common.HttpRequest.prototype.wireEvent_ = function() {
  ...
  goog.events.listen(this.xhrIo_, goog.net.EventType.READY_STATE_CHANGE, function(b) {
    var c = b.target.getReadyState(), d = b.target.getStatus();
    if (1 === c || 2 === c) {
      a.onReadyStateChange_(c, 0, 0);
    } else {
      if (200 === d && 3 === c) {
        d = b.target.getResponseText(), b = b.target.xhr_.getResponseHeader("Content-Length"), a.onReadyStateChange_(c, d.length, b);
      } else {
        if (200 === d && 4 === c) {
          a.onReadyStateChange_(c, 0, 0);
        }
...
WebMessenger.common.HttpRequest.prototype.onComplete_ = function(a) {
  var b = a.target;
  a = null;
  if (b.isSuccess()) {
    try {
      ...
      b = 0;
      window.console && (b = (new Date).getTime() - this.requestTime_, window.console.log("Finish requested time:" + b + " msec"));
      this.onSuccessCallback_(a, this.sender_);
    } catch (d) {
      window.console.log(d.stack), this.onErrorCallback_(a, this.sender_);
    }
  } else {
    if (goog.net.ErrorCode.ABORT === b.getLastErrorCode()) {
      this.onAbortCallback_("request was aborted : " + b.getLastError(), this.sender_);
    } else {
      a = b.getResponseText(), this.onErrorCallback_(a, this.sender_);
    }
  }
};
WebMessenger.common.HttpRequest.prototype.addHeader = function(a, b) {
  this.headers_.set(a, b);
};
WebMessenger.common.HttpRequest.prototype.request = function(a, b, c, d, e, f, g) {
  goog.isFunction(d) && (this.onSuccessCallback_ = d);
  goog.isFunction(e) && (this.onErrorCallback_ = e);
  goog.isFunction(f) && (this.onAbortCallback_ = f);
  goog.isFunction(g) && (this.onReadyStateChange_ = g);
  this.cancel();
  this.requestTime_ = (new Date).getTime();
  this.xhrIo_.send(a, b || "POST", goog.json.serialize(c), this.headers_);
};
WebMessenger.common.HttpRequest.prototype.requestWithoutHandler = function(a, b, c, d, e, f, g) {
  goog.isFunction(d) && (this.onSuccessCallback_ = d);
  goog.isFunction(e) && (this.onErrorCallback_ = e);
  goog.isFunction(f) && (this.onAbortCallback_ = f);
  goog.isFunction(g) && (this.onReadyStateChange_ = g);
  goog.net.XhrIo.send(a, this.onComplete_.bind(this), b || "POST", goog.json.serialize(c), this.headers_);
};
WebMessenger.common.HttpRequest.prototype.cancel = function() {
  if (this.xhrIo_.isActive()) {
    try {
      ...
      this.onErrorCallback_("A fatal error occured while sending request : " + a, this.sender_);
    }
  }
};
WebMessenger.common.HttpRequest.prototype.postRequest = function(a, b, c) {
  c = c || "post";
  var d = document.createElement("form");
  d.setAttribute("method", c);
  d.setAttribute("action", a);
  for (var e in b) {
    b.hasOwnProperty(e) && (a = document.createElement("input"), a.setAttribute("type", "hidden"), a.setAttribute("name", e), a.setAttribute("value", b[e]), d.appendChild(a));
  }
...
WebMessenger.events.OnReceiveHistory = function(a) {
  this.messageList = a;
...
WebMessenger.services.ChatService.prototype.sendMessage = function(a, b) {
  this.socket_.emit("send_message", {name:a, message:b});
};
...
WebMessenger.services.ChatService.prototype.httpResponseSuccess_ = function(a) {
  a = new WebMessenger.events.OnReceiveHistory(a);
  this.dispatchEvent_(a);
};
WebMessenger.models = {};
WebMessenger.models.DataModel = function() {
};
goog.exportSymbol("WebMessenger.models.DataModel", WebMessenger.models.DataModel);
WebMessenger.models.DataModel.prototype.userName = null;
WebMessenger.App = function() {
  ...
  goog.events.listen(this.chatService_.getEventTarget(), [WebMessenger.enums.EventType.ON_RECEIVE_HISTORY], this.OnReceiveHistory_, !1, this);
  $("#message").on("keydown", function(a) {
    13 === a.keyCode && (this.chatService_.sendMessage(this.dataModel_.userName, a.target.value), a.target.value = "");
  }.bind(this));
  this.initDialog_();
...
WebMessenger.App.prototype.initDialog_ = function() {
  this.$dialog_ = $("#dialog");
  var a = $('<div style="display: inline-flex;"></div>'), b = $('<span style="padding-right: 10px; line-height: 25px;">Your name : </span>'), c = $("<div></div>");
  this.$dialogInput_ = $("<input></input>");
  this.$dialogInput_.on("keydown", this.onNameEnterFromInput_.bind(this));
  c.append(this.$dialogInput_);
  a.append(b);
  a.append(c);
  this.$dialog_.append(a);
  a = [{text:"Ok", click:this.onNameEnter_.bind(this)}];
  this.$dialog_.dialog({title:"Please enter your name", dialogClass:"no-close", width:400, resizable:!1, dragable:!0, closeText:"hide", modal:!0, closeOnEscape:!1, buttons:a, autoOpen:!1});
};
WebMessenger.App.prototype.OnReceiveMessage_ = function(a) {
  this.addChatMessage(a.name, a.message);
  $(".panel-body").animate({scrollTop:$(".chat").height()}, 800);
};
WebMessenger.App.prototype.OnReceiveHistory_ = function(a) {
  a = a.messageList;
  for (var b in a) {
    var c = a[b];
    this.addChatMessage(c.name, c.message);
  }
  $(".panel-body").animate({scrollTop:$(".chat").height()}, 1E3);
};
WebMessenger.App.prototype.onNameEnterFromInput_ = function(a) {
  if (13 === a.keyCode) {
    this.onNameEnter_(a);
  }
};
WebMessenger.App.prototype.onNameEnter_ = function(a) {
  "" !== this.$dialogInput_.val() ? (this.dataModel_.userName = this.$dialogInput_.val(), this.$dialog_.dialog("close"), this.chatService_.getHistory()) : setTimeout(this.$dialog_.dialog("open"), 1E3);
};
WebMessenger.App.prototype.addChatMessage = function(a, b) {
  var c = "";
  this.dataModel_.userName === a ? (c = c + '<li class="right clearfix"><span class="chat-img pull-right">' + ('<img src="https://placeholdit.imgix.net/~text?txtsize=16&bg=2d4daf&txtclr=fff&w=60&h=50&txt=' + a + '" alt="User Avatar" class="img-circle" />'), c += '</span><div class="chat-body clearfix"><div class="header">', c += '<small class=" text-muted"><span class="">&nbsp;</span></small><strong class="pull-right primary-font">' + a + "</strong></div>", c += '<p style="float:right;">' + b + "</p></div></li>") : 
  (null == this.otherPeopleColorMap_[a] && (this.otherPeopleColorMap_[a] = this.otherPeopleColorList_[this.otherPeopleColorIndex_], this.otherPeopleColorIndex_ < this.otherPeopleColorList_.length - 1 && this.otherPeopleColorIndex_++), c += '<li class="left clearfix"><span class="chat-img pull-left">', c += '<img src="https://placeholdit.imgix.net/~text?txtsize=16&bg=' + this.otherPeopleColorMap_[a] + "&txtclr=000&w=60&h=50&txt=" + a + '" alt="User Avatar" class="img-circle" />', c += '</span><div class="chat-body clearfix"><div class="header">', 
  c += '<strong class="primary-font">' + a + "</strong></div>", c += "<p>" + b + "</p></div></li>");
  for ($("#messages").append($(c));100 < $("#messages li").length;) {
